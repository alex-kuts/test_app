...
module ActiveAdmin
  ...
  class BaseController < ::InheritedResources::Base
    ...
    custom_actions :resource=>:deleteimg
    ...
    def create
      create! do |format|
        if !resource.new_record?
          format.html { redirect_to params[:save] ? edit_resource_url : collection_url }
        end
      end
    end
    ...
    def deleteimg
      @object = resource
      @object.send("remove_#{params[:field]}!")
      respond_to do |format|
        format.json {render :json => @object.send(params[:field]+'?') ?
            {:type => 'error'} :
            {:type => 'ok' } }
      end
    end
