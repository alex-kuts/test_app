$(document).ready(function() {
	...
	$('#mass_upl').click(function(event) {
		event.preventDefault();
		if($(this).hasClass('active')) {
			$('#workfield').slideUp(300);
			$(this).removeClass('active');		
		} else {
			$('#workfield').slideDown(300);	
    ...
    fileInput.damnUploader({
        ...
        onLimitExceeded: function() {
            alert('Допустимое кол-во файлов уже выбрано');
        },
        ...
        var uploadItem = {
            ...
            onComplete: function(successfully, data, errorCode) {
                if(successfully) {
                    ...
                    if(!this.cancelled) {
                        alert('Файл `'+this.file.name+'`: ошибка при загрузке. Код: '+errorCode+'</span>');
                    }
                }
            }
        };
    ...
    $("#upload-all").click(function(event) {
    	event.preventDefault();
    ...
    $("#cancel-all").click(function(event) {
        ...
        imgSize = 0;
        updateInfo();
        //showAlertMess('Все загрузки завершены')
        imgList.empty();
    });
    ...
    if(!$.support.fileSelecting) {
        alert('Ваш браузер не поддерживает выбор файлов (загрузка будет осуществлена обычной отправкой формы), если бы ты использовал хороший браузер, файлы можно было бы перетаскивать прямо в область ниже!');
    } else {
        if(!$.support.fileReading) {
            alert('* Ваш браузер не умеет читать содержимое файлов (миниатюрки не будут показаны)');
        }
        if(!$.support.uploadControl) {
            alert('* Ваш браузер не умеет следить за процессом загрузки (progressbar не работает)');
        }
        if(!$.support.fileSending) {
            alert('* Ваш браузер не поддерживает объект FormData (отправка с ручной формировкой запроса)');
        }
